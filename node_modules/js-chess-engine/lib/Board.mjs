...
export default class Board {
     ...
     * @param {String} from 
     * @param {String} to 
     * @param {String} specMove 
     * @returns {{ moves: Array.<Array<String>>, captured: String }}
     */
    move (from, to, specMove = "") {
        // Move logic
        const chessmanFrom = this.getPiece(from)
        const chessmanTo = this.getPiece(to)

        var info = {
            isSpec: Boolean(specMove),
            moves: [[from,to]],
            captured: []
        }
        ...
        if (this.isPawn(chessmanFrom) && to === this.configuration.enPassant) {
            var p = downByColor(to, this.getPlayingColor())
            info.captured.push(p)
        ...
        if (this.isKing(chessmanFrom)) {
            if (from === 'E1' && to === 'C1') return this.move('A1', 'D1',[from,to,""])
            if (from === 'E8' && to === 'C8') return this.move('A8', 'D8',[from,to,""])
            if (from === 'E1' && to === 'G1') return this.move('H1', 'F1',[from,to,""])
            if (from === 'E8' && to === 'G8') return this.move('H8', 'F8',[from,to,""])
        }
        ...
        if (info.isSpec){
            info.moves.push(specMove.slice(0,2))
        }
